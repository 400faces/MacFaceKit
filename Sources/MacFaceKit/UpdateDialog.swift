...
// The shared update-flow dialog — every update state is one `UpdateDialog`: the icon never resizes, the
// header / sub-header / button row hold their size and place, and only the middle (release notes or a
// progress bar) changes, so the flow morphs smoothly instead of jumping between bespoke layouts. It is
// Sparkle-FREE: each app's Sparkle user driver maps its callbacks to a `UpdateDialog` state and hosts it.
// Parameterized by the app's name (strings) + icon, so both apps show the same dialog with their own brand.
...
public struct UpdateActionButton: View {
    ...
    public init(title: String, systemImage: String? = nil, primary: Bool = false, width: CGFloat? = nil,
...
public struct ReleaseNotesSection: View {
    private let items: [String]
    @Binding private var expanded: Bool

    public init(items: [String], expanded: Binding<Bool>) {
        self.items = items
...
/// changes between states. `icon` is the app's own icon (constant across states — set once by the driver).
public struct UpdateDialog: View {
    public struct Action {
        let title: String
        var icon: String?
        let run: () -> Void
        public init(title: String, icon: String? = nil, run: @escaping () -> Void) {
            self.title = title
            self.icon = icon
            self.run = run
        }
    ...
    /// The app's icon — applied once by the driver so every state shows the same brand.
    public var icon: NSImage?

    public init(header: String, badge: String? = nil, subheader: String? = nil, middle: Middle = .none,
                notesExpanded: Binding<Bool> = .constant(true), primary: Action? = nil,
                secondary: Action? = nil, icon: NSImage? = nil) {
        self.header = header
        self.badge = badge
    ...
    /// Returns a copy with the app icon set — the driver/gallery apply their app's icon once, so every
    /// state shows the same brand without threading it through all seven factories.
    public func icon(_ image: NSImage?) -> UpdateDialog {
        var copy = self
        copy.icon = image
        return copy
    }
...
/// One authority for every update-state dialog, so an app's live driver and its dev gallery render the
/// exact same thing. Parameterized by `appName` (its strings) — the driver applies the app `icon` once.
extension UpdateDialog {
    public static func permission(appName: String, onAllow: @escaping () -> Void,
                                  onDecline: @escaping () -> Void) -> UpdateDialog {
        UpdateDialog(
            header: "Check for updates automatically?",
            subheader: "\(appName) can check for new versions on its own. Updates are signed and installed in place.",
            primary: .init(title: "Check", run: onAllow),
            secondary: .init(title: "Not Now", run: onDecline))
    }

    public static func checking(onCancel: @escaping () -> Void) -> UpdateDialog {
        UpdateDialog(
    ...
    public static func progress(appName: String, heading: String, version: String, fraction: Double?,
                                onCancel: (() -> Void)?) -> UpdateDialog {
        UpdateDialog(
            header: heading,
            subheader: "\(appName) \(version)",
    ...
    public static func available(appName: String, version: String, currentVersion: String, notes: [String],
                                 notesExpanded: Binding<Bool>,
                                 onInstall: @escaping () -> Void,
                                 onRemindLater: @escaping () -> Void) -> UpdateDialog {
        UpdateDialog(
            header: "A new version is available!",
            subheader: "\(appName) \(version) is available — you have \(currentVersion).",
            middle: .notes(notes),
    ...
    public static func ready(appName: String, version: String, onRestart: @escaping () -> Void) -> UpdateDialog {
        UpdateDialog(
            header: "Update ready",
            badge: "checkmark.circle.fill",
            subheader: "\(appName) \(version) has been downloaded.",
            primary: .init(title: "Restart Now", run: onRestart))
    }

    public static func upToDate(appName: String, version: String, onOK: @escaping () -> Void) -> UpdateDialog {
        UpdateDialog(
            header: "You're up to date!",
            subheader: "\(appName) \(version) is the newest version.",
            primary: .init(title: "OK", run: onOK))
    }

    public static func error(message: String, onOK: @escaping () -> Void) -> UpdateDialog {
