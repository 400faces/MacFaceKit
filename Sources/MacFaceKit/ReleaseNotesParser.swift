...
/// How the update feed labeled a set of embedded release notes (Sparkle's `SUAppcastItem
/// .itemDescriptionFormat` string — passed in raw so this stays Sparkle-free).
public enum ReleaseNotesFormat: Equatable {
    case markdown
    case plainText
    case html

    /// Map the feed's format string. A nil or unrecognized value defaults to HTML, matching Sparkle's
    /// own fallback for legacy appcasts, so old feeds keep working.
    public init(sparkleFormat raw: String?) {
...
/// `releaseNotesLink` (HTML) — and both funnel through here so the list is produced identically. The
/// caller extracts the Sparkle payload; this type is Sparkle-free so it lives in the shared kit.
///
/// The contract is a *flat bullet list* (notes are authored that way): one item per line, with leading
/// bullet glyphs and the common inline emphasis markers stripped. It is deliberately not a full markdown
/// renderer — nested structure and headers are out of scope by design.
public enum ReleaseNotesParser {
    /// Bullet items from raw markup, or `nil` when nothing renderable remains.
    public static func items(from markup: String, format: ReleaseNotesFormat) -> [String]? {
        let plain: String
        switch format {
        case .html:
    ...
    public static func items(from data: Data, format: ReleaseNotesFormat = .html) -> [String]? {
